...
if len(sys.argv) < 5:
    print("Usage: python crop_with_mask.py <volume_path> <mask_path> <out_dir> <out_name> [margin]")
    sys.exit(1)
...
out_dir = sys.argv[3]
out_name = sys.argv[4]
margin = int(sys.argv[5]) if len(sys.argv) > 5 else 10  # default margin = 10 voxels
...
# Update affine matrix for new origin
# The origin needs to be shifted by the min coordinates
new_affine = vol_img.affine.copy()
min_coords_physical = vol_img.affine @ np.array([minx, miny, minz, 1])
new_affine[:3, 3] = min_coords_physical[:3]

# Save cropped versions
vol_crop_img = nib.Nifti1Image(vol_crop, new_affine, vol_img.header)
mask_crop_img = nib.Nifti1Image(mask_crop, new_affine, mask_img.header)
