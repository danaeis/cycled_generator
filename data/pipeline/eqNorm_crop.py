...
# =================== CONFIG ===================
INPUT_DIR = Path("../../ncct_cect/vindr_ds/registered_output")
OUTPUT_DIR = Path("../../ncct_cect/vindr_ds/registered_aligned")
# INPUT_DIR = Path("../../ncct_cect/vindr_ds/deformable_aligned_equalized_registered_bspline")
# OUTPUT_DIR = Path("../../ncct_cect/vindr_ds/equalized_deforable_registered_aligned")
# tag = 'deformable' 
tag = 'registered'
BODY_THRESHOLD = -700                              
...
def process_case(case_path: Path):
    ...
    print('='*70) 
    # print(f"  Looking for volumes with tag: '{tag}'")
    image_files = sorted([f for f in case_path.glob(f"*_{tag}.nii.gz") if "_seg" not in f.name])
    if not image_files:
    ...
    for vol_data in volumes_data:
        try:
            ...
            vol_cropped_out = sitk.GetImageFromArray(cropped_np, isVector=False)
            # Copy physical properties EXCEPT size
            vol_out.SetSpacing(vol_sitk.GetSpacing())
            vol_out.SetDirection(vol_sitk.GetDirection())
            vol_cropped_out.SetSpacing(vol_sitk.GetSpacing())
            vol_cropped_out.SetDirection(vol_sitk.GetDirection())
            ...
            vol_cropped_out.SetOrigin(new_origin.tolist())
            ...
            new_name = img_file.name.replace(f"_{tag}.nii.gz", "_cropped.nii.gz")
            out_path_cropped = out_case_dir / new_name
            sitk.WriteImage(vol_cropped_out, str(out_path_cropped))
            print(f"    ✅ Saved cropped: {new_name}")

            new_name = img_file.name.replace(f"_{tag}.nii.gz", f"_{tag}_norm.nii.gz")
            ...
            # ===================================================================
            for suffix in [f"_{tag}_seg.nii.gz", "_seg.nii.gz"]:
                seg_path = img_file.parent / img_file.name.replace(f"_{tag}.nii.gz", suffix)
                if seg_path.exists():
