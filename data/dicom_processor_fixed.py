...
class StandardizedDICOMProcessor:
    ...
    def resample_to_target_spacing(self, image: sitk.Image) -> sitk.Image:
        ...
        # Get appropriate background value
        img_array = sitk.GetArrayFromImage(image)
        background_value = float(img_array.min())  # Use minimum value as background
        ...
        resampler.SetDefaultPixelValue(background_value)  # Fixed: use proper background value
        
        # Choose appropriate interpolation
        if image.GetPixelID() in [sitk.sitkUInt8, sitk.sitkInt8, sitk.sitkUInt16, sitk.sitkInt16, sitk.sitkInt32]:
            # For integer types, use linear interpolation
            resampler.SetInterpolator(sitk.sitkLinear)
        else:
            # For float types, use linear interpolation
            resampler.SetInterpolator(sitk.sitkLinear)
        
        resampled_image = resampler.Execute(image)
        
        # Debug: Check resampled image
        resampled_array = sitk.GetArrayFromImage(resampled_image)
        logger.info(f"Resampling complete. New spacing: {resampled_image.GetSpacing()}")
        logger.info(f"Resampled pixel range: [{resampled_array.min():.2f}, {resampled_array.max():.2f}]")
        
        # Additional check for constant values
        if resampled_array.max() == resampled_array.min():
            logger.error(f"Resampled image has constant values: {resampled_array.max()}")
            # Try with nearest neighbor interpolation as fallback
            logger.info("Trying nearest neighbor interpolation as fallback...")
            resampler.SetInterpolator(sitk.sitkNearestNeighbor)
            resampled_image = resampler.Execute(image)
            resampled_array = sitk.GetArrayFromImage(resampled_image)
            logger.info(f"Fallback result - pixel range: [{resampled_array.min():.2f}, {resampled_array.max():.2f}]")
    ...
    def convert_standardized_dicom_to_nifti(
        ...
        try:
            ...
            # Load and standardize the DICOM series
...
def process_series_list_to_standardized_nifti(
    ...
    for i, series_info in enumerate(series_data, 1):
        try:
            ...
            output_filename = f"{study_uid}_{series_uid}_{phase}_standardized.nii.gz"
    ...
    def debug_dicom_series(self, dicom_path: str) -> Dict:
        """
        Debug function to inspect DICOM series and identify potential issues.
        ...
        try:
            ...
            if not os.path.exists(dicom_path):
                debug_info['potential_issues'].append(f"Directory does not exist: {dicom_path}")
                return debug_info
            
            # Count files
            all_files = [f for f in os.listdir(dicom_path) if os.path.isfile(os.path.join(dicom_path, f))]
            ...
            for file in all_files:
                ...
                try:
                    ds = pydicom.dcmread(file_path)
                    if hasattr(ds, 'SOPClassUID'):
                        valid_dicoms.append(file_path)
                        if hasattr(ds, 'pixel_array'):
                            pixel_data_count += 1
                except Exception as e:
                    continue
            
            debug_info['valid_dicom_files'] = len(valid_dicoms)
            debug_info['pixel_data_files'] = pixel_data_count
            
            if len(valid_dicoms) == 0:
                debug_info['potential_issues'].append("No valid DICOM files found")
                return debug_info
            
            # Examine first DICOM file
            first_dicom = pydicom.dcmread(valid_dicoms[0])
            
            # Check for scaling
            if hasattr(first_dicom, 'RescaleSlope') or hasattr(first_dicom, 'RescaleIntercept'):
                debug_info['has_scaling'] = True
                debug_info['scaling_info'] = {
                    'rescale_slope': getattr(first_dicom, 'RescaleSlope', 1.0),
                    'rescale_intercept': getattr(first_dicom, 'RescaleIntercept', 0.0)
                }
            
            # Check spacing info
            if hasattr(first_dicom, 'PixelSpacing'):
                debug_info['spacing_info']['pixel_spacing'] = list(first_dicom.PixelSpacing)
            if hasattr(first_dicom, 'SliceThickness'):
                debug_info['spacing_info']['slice_thickness'] = float(first_dicom.SliceThickness)
            
            # Check orientation
            if hasattr(first_dicom, 'ImageOrientationPatient'):
                debug_info['orientation_info']['image_orientation_patient'] = list(first_dicom.ImageOrientationPatient)
            if hasattr(first_dicom, 'ImagePositionPatient'):
                debug_info['orientation_info']['image_position_patient'] = list(first_dicom.ImagePositionPatient)
            
            # Check pixel data
            if hasattr(first_dicom, 'pixel_array'):
                pixel_array = first_dicom.pixel_array
                debug_info['sample_pixel_stats'] = {
                ...
                if pixel_array.max() == pixel_array.min():
                    debug_info['potential_issues'].append(f"Constant pixel values: {pixel_array.max()}")
                
                if len(np.unique(pixel_array)) < 10:
                    debug_info['potential_issues'].append(f"Very few unique pixel values: {len(np.unique(pixel_array))}")
            
            # Try loading with SimpleITK
            try:
                image = self._load_dicom_with_fallbacks(dicom_path)
                img_array = sitk.GetArrayFromImage(image)
                debug_info['sitk_load_success'] = True
                debug_info['sitk_pixel_stats'] = {
                    'shape': img_array.shape,
                    'min': float(img_array.min()),
                    'max': float(img_array.max()),
                    'mean': float(img_array.mean())
                }
                debug_info['sitk_spacing'] = image.GetSpacing()
                debug_info['sitk_origin'] = image.GetOrigin()
                debug_info['sitk_direction'] = image.GetDirection()
                
            except Exception as e:
                debug_info['sitk_load_success'] = False
                debug_info['sitk_error'] = str(e)
                debug_info['potential_issues'].append(f"SimpleITK loading failed: {str(e)}")
            
            return debug_info
            
        except Exception as e:
            debug_info['potential_issues'].append(f"Debug analysis failed: {str(e)}")
            return debug_info
    
    def print_debug_report(self, dicom_path: str) -> None:
        """
        Print a detailed debug report for a DICOM series.
        """
        debug_info = self.debug_dicom_series(dicom_path)
        
        print(f"\n=== DICOM DEBUG REPORT ===")
        print(f"Path: {debug_info['dicom_path']}")
        print(f"Files found: {debug_info['files_found']}")
        print(f"Valid DICOM files: {debug_info['valid_dicom_files']}")
        print(f"Files with pixel data: {debug_info['pixel_data_files']}")
        
        if debug_info['has_scaling']:
            print(f"\nScaling info:")
            print(f"  Rescale slope: {debug_info['scaling_info']['rescale_slope']}")
            print(f"  Rescale intercept: {debug_info['scaling_info']['rescale_intercept']}")
        
        if debug_info['spacing_info']:
            print(f"\nSpacing info:")
            for key, value in debug_info['spacing_info'].items():
                print(f"  {key}: {value}")
        
        if debug_info['sample_pixel_stats']:
            print(f"\nPixel data statistics:")
            stats = debug_info['sample_pixel_stats']
        ...
        if 'sitk_load_success' in debug_info:
            print(f"\nSimpleITK loading: {'SUCCESS' if debug_info['sitk_load_success'] else 'FAILED'}")
            if debug_info['sitk_load_success']:
                print(f"  Shape: {debug_info['sitk_pixel_stats']['shape']}")
                print(f"  Range: [{debug_info['sitk_pixel_stats']['min']}, {debug_info['sitk_pixel_stats']['max']}]")
                print(f"  Spacing: {debug_info['sitk_spacing']}")
            else:
                print(f"  Error: {debug_info['sitk_error']}")
...
# Add a convenience function for debugging
def debug_problematic_series(series_data: List[Dict], processor: StandardizedDICOMProcessor) -> None:
    """
    Debug multiple series to identify problematic ones.
    """
    print("Debugging DICOM series...")
    
    for i, series_info in enumerate(series_data[:5]):  # Debug first 5 series
        print(f"\n[{i+1}] Debugging: {series_info['study_uid']}_{series_info['phase']}")
        processor.print_debug_report(series_info['series_path'])
...
def main():
    """
    Example usage of the complete standardization workflow with debugging options.
    """
    # Define paths
    batch_dir = "/path/to/your/batch/directory"
    ...
    # complete_standardization_workflow(
    #     batch_dir=batch_dir,
    #     labels_csv=labels_csv,
    #     output_base_dir=output_base_dir,
    ...
    # OPTION 3: Debug multiple series from your dataset
    # First, get series data
    # temp_pkl = "temp_series_data.pkl"
    # series_data = save_dicom_paths_with_standardization(
    #     batch_dir=batch_dir,
    #     labels_csv=labels_csv,
    #     output_pkl=temp_pkl,
    ...
    # )
    # 
    # # Then debug problematic ones
    # debug_problematic_series(series_data, processor)
    
    print("Update the paths and uncomment the desired option")

# Common solutions for 'all values 1' issue:
"""
TROUBLESHOOTING GUIDE FOR 'ALL VALUES 1' ISSUE:

1. **DICOM Pixel Scaling Issue:**
   - Some DICOM files have RescaleSlope=0 or incorrect scaling
   - The updated code now handles this automatically

2. **Wrong Default Pixel Value:**
   - Old code used image.GetPixelIDValue() which could be 1
   - New code uses actual minimum pixel value as background

3. **Resampling Transform Issue:**
   - Image might be shifted outside the new coordinate system
   - New code includes better error checking and fallback methods

4. **DICOM Loading Problem:**
   - Some DICOM series don't load properly with standard methods
   - New code has 3 fallback loading methods

TO DEBUG YOUR SPECIFIC ISSUE:
1. Run: processor.print_debug_report("/path/to/your/problematic/dicom/folder")
2. Look for warnings about constant pixel values
3. Check if scaling info shows unusual values
4. Try the fallback loading methods

QUICK FIXES TO TRY:
- Set overwrite=True to regenerate existing files
- Check if your DICOM files have pixel_array data
- Verify DICOM files aren't corrupted
- Try with a different series from the same study
"""
